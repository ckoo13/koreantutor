...
// ── Quiz Mode ──────────────────────────────────────────────────────
function QuizMode() {
  const { quizPool, selectedUnitId } = useAppData();
  const { user } = useAuth();
  ...
  const handleNext = () => {
    if (idx < questions.length - 1) {
      ...
      if (user && selectedUnitId) {
        saveUnitScore(user.id, selectedUnitId, finalScore).then(() => setScoreSaved(true));
      }
    }
  };
    ...
    return (
      <div style={{ maxWidth: 540, margin: "0 auto", animation: "fadeIn 0.5s ease", paddingTop: 20 }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <p style={{ fontFamily: font.body, fontSize: 16, color: palette.textDim, margin: "0 0 12px 0" }}>
            {pct >= 80 ? "훌륭합니다! 고급 수준의 실력이시네요." : pct >= 50 ? "좋은 출발입니다. 복습하면 더 좋아질 거예요." : "어려웠죠? 각 섹션을 복습해 보세요!"}
          </p>
          {selectedUnitId && scoreSaved && (
            <p style={{ fontFamily: font.body, fontSize: 14, color: pct >= 70 ? palette.correct : palette.accentDim, margin: "0 0 24px 0" }}>
              {pct >= 70 ? "✓ 유닛 통과! 다음 유닛이 잠금 해제됩니다." : `유닛 통과까지 ${70 - pct}점 더 필요합니다. (통과 기준: 70점)`}
            </p>
          )}
        </div>
